...
export function TagsFilter({
  ...
  const handleDeselectIndex = (index: string) => {
  ...
  return (
    <div className={Styles.container}>
      <div className={Styles.main}>
        ...
        <Widget title={t('widgetTitles.tags')} data={
            <div>{availableTags.map(tag => <div key={tag} className={Styles.tag} onClick={() => handleSelectTag(tag)}>{tag}</div>)}</div>
        } />
        <Widget title={t('widgetTitles.indexes')} data={
            <div>{availableIndexes.map(index => <div key={index} className={Styles.tag} onClick={() => handleSelectIndex(index)}>{index}</div>)}</div>
        } />
        
        <Widget title={t('widgetTitles.search')} data={
            <div>
              <label htmlFor="problemName">{t('filters.problemName')}: </label>
              <input type="text" name="problemName" value={problemName} onChange={(e) => onProblemNameChange(e.target.value)} className={Styles.input} />
            </div>
        }/>

        <div className="flex flex-row w-full">
            <div className="flex-1 mr-2">
                <Widget title={t('widgetTitles.ratingSort')} data={
                    <div>
                        <label htmlFor="minRating">{t('filters.min')}: </label>
                        <input type="number" name="minRating" value={minRating || ''} onChange={(e) => handleNumberInputChange(e, onMinRatingChange)} className={Styles.input} />
                        <br/>
                        <label htmlFor="maxRating">{t('filters.max')}: </label>
                        <input type="number" name="maxRating" value={maxRating || ''} onChange={(e) => handleNumberInputChange(e, onMaxRatingChange)} className={Styles.input} />
                    </div>
                }/>
            </div>
            <div className="flex-1">
                <Widget title={t('widgetTitles.pointsSort')} data={
                    <div>
                        <label htmlFor="minPoints">{t('filters.min')}: </label>
                        <input type="number" name="minPoints" value={minPoints || ''} onChange={(e) => handleNumberInputChange(e, onMinPointsChange)} className={Styles.input} />
                        <br/>
                        <label htmlFor="maxPoints">{t('filters.max')}: </label>
                        <input type="number" name="maxPoints" value={maxPoints || ''} onChange={(e) => handleNumberInputChange(e, onMaxPointsChange)} className={Styles.input} />
                    </div>
                }/>
            </div>
        </div>
